...
class RBACService {
    ...
    async syncMissingFeatures() {
        ...
        for (const f of missingFeatures) {
            ...
            if (existing.length === 0) {
                ...
                console.log(`[RBAC] Auto-inserted missing feature: ${f.key}`);
            }
        }
    }
    ...
    async getFeatures() {
        try {
            await this.syncMissingFeatures();
        } catch (e) {
            console.warn('[RBAC] Failed to sync missing features:', e);
        }
